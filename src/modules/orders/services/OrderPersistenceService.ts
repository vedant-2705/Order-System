/**
 * @module OrderPersistenceService
 * @description Handles the atomic write phase of order creation.
 *
 * Called by `CreateOrderUseCase` after validation succeeds.
 * Always receives an existing transaction  it never opens one itself.
 *
 * Write sequence (single transaction, two queries):
 *   1. INSERT into `orders`            `order_number` generated by DB trigger
 *   2. Bulk INSERT into `order_items`  single round-trip via array insert
 *
...
/** Output returned by `OrderPersistenceService.persist()`. */
